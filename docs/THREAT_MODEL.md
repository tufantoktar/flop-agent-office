...
| T20 | The public identity being mistaken for a credential | The DID is public by construction (it embeds a public key). It is committed, printed abbreviated by default, and documented in three places as not a wallet, not trust, not eligibility, not authorisation to publish. `config/settings.py` imports no key machinery, and a test asserts it. | Someone reading `ROOT_AGENT_DID` in config and assuming a signer is loaded. The doctor output reports configuration only and says the capability signer is not loaded. |
...
| T23 | A keystore replaced or tampered with under a correct-looking path | File mode `0600` **and** directory mode `0700` are both enforced; a symlink and non-regular file are refused; repo-local paths are refused; an unencrypted PEM is refused outright with no plaintext fallback. | TOCTOU between the stat and the read. Not closed; the DID check downstream is what catches a swapped key. |
| T24 | Key material reached by auto-discovery | No module searches for a keystore path: every runtime key path comes from explicit `FLOPOFFICE_KEYSTORE` config, and a test greps for `Path.home()` / `expanduser` joined to key-shaped names across all production packages. `doctor` never prints the path or filename and never loads the key. | An operator pointing config at the wrong file. The DID check catches it. |
...
| T26 | A locally signed first announcement being mistaken for publication | `technocore.announcement` records `publish_status=NOT_SENT`, appends a `technocore_message_publish_blocked` row, calls no HTTP send method, and leaves the public host denylist untouched. Documentation says signed preparation is authorship evidence only. | A human may later choose to publish the exact message. That requires a separate explicit approval step. |
| T27 | First-message proof leaking secrets into the ledger | The proof rows store public DID, room, nonce, hashes, canonical text, Ed25519 signature, algorithm/profile, and NOT_SENT state only; the ledger scanner still runs on every string. Tests assert no passphrase, key path or private key marker appears in output/meta. | A future change could add more metadata; activity validation and secret scanning are the backstop. |
| T28 | Nonce reuse during announcement preparation | M1.6 reserves the room nonce through `NonceStore` before signing. Explicit nonces are accepted only if they exceed the stored local counter; non-increasing values fail before signing. | Separate machines sharing one DID and separate ledgers can still collide. Do not split one root DID across machines. |
| T29 | One public publish approval becoming a general write capability | M1.7 keeps `PUBLIC_HOSTS` in place and adds `OneTimePublicWriteGate`, scoped to one public host, room, canonical message hash and nonce. The gate is consumed before the request and closed in `finally`, so normal client calls still fail and retries cannot reuse it. | Code with authority to create another gate could attempt another send. That creation is the reviewed approval boundary. |
| T30 | Ambiguous HTTP result causing duplicate publication | The M1.7 helper sends once, records `AMBIGUOUS_OR_FAILED` on transport errors and raises. It does not retry or reserve another nonce automatically. | A human may need to inspect Technocore manually before deciding whether any follow-up is safe. |
